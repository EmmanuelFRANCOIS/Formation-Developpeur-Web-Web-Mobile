...
Class ModProductsByRandom {
   ...
   *        ]
   */
  private function getProductsByRandom( $options = null ) {
  ...
  /**
   * @function genProductsByRandom()
   * @summary  return Html generated code for a module with n popular products
   *           by universe, by category and/or by brand
   * @param    $options = [
   *             'display'     => ['H-Blocks', 'V-Blocks', 'table'] (default : 'H-Blocks')
   *             'moduleTitle' => ['module title'] (default : 'Livres par hazard')
   *             'universe_id' => [id of the Universe] (default : null)
   *             'category_id' => [id of the Category] (default : null)
   *             'brand_id'    => [id of the Brand]    (default : null)
   *             'mode'        => ['created' or 'modified'] (default: 'created')
   *             'nb'          => [(int) # of products to return] (default : 5)
   *             'nb_by_row'   => [(any) # of products by row] (default : auto)
   *           ]
   */
  public static function genProductsByRandom( $options = null ) {
    ...
    // Check $nb value
    $nb = $options['nb'] > 0 ? $options['nb'] : 5;
    switch( true ) {
      case in_array( $nb, range(1,6)  ): $nbH = $nb; break;
      case in_array( $nb, range(7,12) ): $nbH = ceil($nb / 2); break;
      default:
        $mods = ['5' => fmod($nb, 5), '6' => fmod($nb, 6)];
        arsort($mods);
        $nbH = array_keys($mods)[0]; 
        break;
    }

    // Get Products from DB Product table
    $products = ModProductsByRandom::getProductsByRandom( $options );
    
    // Generate Html module
  ?>
    <div class="container-fluid">
      <div class="container py-4 my-3">
        <h3 class="text-success module-title">Découvrez quelques <span class="text-uppercase"><?php echo $options['moduleTitle']; ?></span></h3>
        ...
        <div class="my-3 fs-5 text-end">Découvrez quelques <a class="btn btn-success fs-5 fw-bold py-1 px-3 more" href="#"><?php echo $options['moduleTitle'] ?></a></div>
      </div>
    </div>
  <?php
